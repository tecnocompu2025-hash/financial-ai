...
export default function ReportManagerV3({ token }: { token: string }) {
  ...
  return <main className="min-h-screen bg-slate-950 p-5 text-white md:p-8">
    ...
    <div className="mt-4 flex gap-3"><button onClick={() => void downloadFinancialReport(token, "pdf", filters).catch(() => setError("No se pudo exportar el PDF."))} className="rounded bg-slate-800 px-4 py-2">Exportar PDF</button><button onClick={() => void downloadFinancialReport(token, "xlsx", filters).catch(() => setError("No se pudo exportar el Excel."))} className="rounded bg-cyan-500 px-4 py-2 font-semibold text-slate-950">Exportar Excel</button></div>
